...
describe('BooksService', () => {
  ...
  it('updates only supplied fields and retains active-book condition', async () => {
    ...
    const result = await service.update(id, { title: 'Updated', available: false, imageUrl: null });
  ...
  it.each(['author', 'publisher', 'genre'] as const)('refuses update with missing %s', async (model) => {
    prisma[model].findUnique.mockResolvedValue(null);
    await expect(service.update(id, dto)).rejects.toBeInstanceOf(BadRequestException);
    expect(prisma.book.update).not.toHaveBeenCalled();
  });

  it('rejects empty updates', async () => {
    await expect(service.update(id, {})).rejects.toBeInstanceOf(BadRequestException);
    expect(prisma.book.update).not.toHaveBeenCalled();
  });
  ...
  it.each(['missing', 'already deleted'])('returns the same 404 when deleting a %s book', async () => {
    prisma.book.update.mockRejectedValue(prismaError('P2025'));
    await expect(service.remove(id)).rejects.toThrow(new NotFoundException('Book not found'));
  });

  it('maps concurrent foreign-key failure to 400', async () => {
    prisma.book.create.mockRejectedValue(prismaError('P2003'));
    await expect(service.create(dto)).rejects.toThrow(new BadRequestException('Invalid author, publisher or genre reference'));
  });
  ...
  it.each([new Error('private details'), prismaError('P2024')])('hides unexpected persistence errors', async (error) => {
    ...
    await expect(service.create(dto)).rejects.toThrow(new InternalServerErrorException('Unable to process book request'));
  });
});
